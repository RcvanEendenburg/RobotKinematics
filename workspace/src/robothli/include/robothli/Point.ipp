...
template<typename T, std::size_t N>
Point<T, N>::Point(Point <T, N> &&otherPoint) noexcept : data(std::move(otherPoint.data))
{
}
    ...
    else if (std::is_floating_point_v<T>)
    {
        for (std::size_t i = 0; i < N; ++i)
            if (!(std::abs(data[i] - rhs.data[i]) <= std::numeric_limits<T>::epsilon()*std::abs(data[i] + rhs.data[i])
                || std::abs(data[i] - rhs.data[i]) < std::numeric_limits<T>::min()))
                return false;
    }
    return true;
}
...
template<typename T, std::size_t N>
T &
Point<T, N>::operator[](std::size_t index)
{
    assert(index < N);
    return data[index];
}
...
template<typename T, std::size_t N>
const T &
Point<T, N>::operator[](std::size_t index) const
{
    assert(index < N);
    return data[index];
}
...
template<typename T, std::size_t N>
Point <T, N> &
Point<T, N>::operator=(Point <T, N> &&rhs) noexcept
{
    if (this!=&rhs)
...
template<typename T, std::size_t N>
const T &
Point<T, N>::get(std::size_t index) const
{
    assert(index < N);
    return data[index];
}
...
template<typename T, std::size_t N>
void
Point<T, N>::set(std::size_t index, const T &value)
{
    assert(index < N);
    data[index] = value;
}
...
template<typename T, std::size_t N>
const T
Point<T, N>::dotProduct(const Point <T, N> &rhs) const
{
    T sum = 0;
    for (std::size_t i = 0; i < N; ++i)
...
template<typename T, std::size_t N>
const T
Point<T, N>::magnitude(const Point <T, N> &rhs) const
{
    T sum = 0;
    for (std::size_t i = 0; i < N; ++i)
        sum += std::pow(rhs[i] - data[i], 2);
    return std::sqrt(sum);
}

template<typename T, std::size_t N>
const T
Point<T, N>::cosineSim(const Point <T, N> &rhs) const
{
