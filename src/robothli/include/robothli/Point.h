...
public:
     ...
     * @param otherPoint The other point.
     */
    Point(Point<T, N> &&otherPoint) noexcept;
     ...
     */
    bool
    operator>(const Point<T, N> &rhs) const;
     ...
     */
    Point<T, N> &
    operator=(Point<T, N> &&rhs) noexcept;
    //@}
     ...
     */
    void
    set(std::size_t index, const T &value);
     ...
     */
    const T
    dotProduct(const Point<T, N> &rhs) const;
    ...
    /**
     * @brief Calculates the magnitude between two points.
     * @param rhs The oter point.
     * @return
     */
    const T
    magnitude(const Point<T, N> &rhs) const;
     ...
     */
    const T
    cosineSim(const Point<T, N> &rhs) const;
    //@}
...
template<typename T, std::size_t N> inline std::ostream &
operator<<(std::ostream &os, const Point<T, N> &rhs)
{
    os << "(";
